...
public class Utils {
	...
	public static Vec3d getRandomCenter(AxisAlignedBB bb) {
		return new Vec3d(bb.minX + (bb.maxX - bb.minX) * 0.8 * Math.random(),
		...
		while (distance > slack) {
			...
			if (wasSneaking) {
				Wrapper.INSTANCE.sendPacket(
						new CPacketEntityAction(Wrapper.INSTANCE.player(), CPacketEntityAction.Action.STOP_SNEAKING));
			}
			Wrapper.INSTANCE.mc().getConnection().getNetworkManager()
					.sendPacket(new CPacketPlayer.Position(startX, startY, startZ, onGround));
			count++;
		}

		if (wasSneaking) {
			Wrapper.INSTANCE.sendPacket(
					new CPacketEntityAction(Wrapper.INSTANCE.player(), CPacketEntityAction.Action.START_SNEAKING));
		}
	...
	public static boolean placeBlockScaffold(final BlockPos pos) {
		final Vec3d eyesPos = new Vec3d(Wrapper.INSTANCE.player().posX,
				Wrapper.INSTANCE.player().posY + Wrapper.INSTANCE.player().getEyeHeight(),
				Wrapper.INSTANCE.player().posZ);
		EnumFacing[] values;
				...
				for (int z = MathHelper.floor(entity.getEntityBoundingBox().minZ); z < MathHelper
					...
					if (block != null && !(block instanceof BlockAir)
							&& (boundingBox = block.getCollisionBoundingBox(BlockUtils.getState(new BlockPos(x, y, z)),
									Wrapper.INSTANCE.world(), new BlockPos(x, y, z))) != null
							&& entity.getEntityBoundingBox().intersects(boundingBox)) {
						return true;
					}
				}
			}
	...
	public static float[] getRotationsNeeded(Entity entity) {
		...
		if ((entity instanceof EntityLivingBase)) {
			...
			diffY = entityLivingBase.posY + entityLivingBase.getEyeHeight()
					- (Wrapper.INSTANCE.mc().player.posY + Wrapper.INSTANCE.mc().player.getEyeHeight());
		} else {
		...
		return new float[] {
				Wrapper.INSTANCE.mc().player.rotationYaw
						+ MathHelper.wrapDegrees(yaw - Wrapper.INSTANCE.mc().player.rotationYaw),
				Wrapper.INSTANCE.mc().player.rotationPitch
						+ MathHelper.wrapDegrees(pitch - Wrapper.INSTANCE.mc().player.rotationPitch) };
	}
}
