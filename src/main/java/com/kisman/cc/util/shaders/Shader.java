...
public abstract class Shader {
    ...
    public Shader(final String fragmentShader) {
        ...
        try {
            final InputStream vertexStream = this.getClass().getResourceAsStream("/assets/kismancc/shaders/vertex.vert");
            vertexShaderID = this.createShader(IOUtils.toString(vertexStream), 35633);
            IOUtils.closeQuietly(vertexStream);
            final InputStream fragmentStream = this.getClass().getResourceAsStream("/assets/kismancc/shaders/fragment/" + fragmentShader);
